...
def test_accepted_getin(test_client, user_tokens):
    ...
    body = {"token": user1, "supermarket_id": supermarket_id}
...
def test_full_supermarket(test_client, user_tokens):
    ...
    body = {"token": user2, "supermarket_id": supermarket_id}
    response = test_client.post('/getin',  headers={'Authorization': 'JWT ' + auth_user2}, json=body)
    assert response.status_code == 201
    
    body = {"token": user3, "supermarket_id": supermarket_id}
...
def test_booking_not_available(test_client, user_tokens):
    """
    ...
    supermarket_id = db_models.Supermarket.query.all()[0].id
    print(supermarket_id)
    body = {"username": user1, "supermarket_id":  supermarket_id , "shop_time": '2021-02-11 10:20'}
    response = test_client.post('/booking',  headers={'Authorization': 'JWT ' + auth_user1}, json=body)
    assert response.status_code == 201

    auth_user2 = user_tokens[1].get('token')
    user2 = user_tokens[1].get('username')
    body = {"username": user2, "supermarket_id": supermarket_id, "shop_time": '2021-02-11 10:20'}
    response = test_client.post('/booking',  headers={'Authorization': 'JWT ' + auth_user2}, json=body)
    assert response.status_code == 201

    auth_user3 = user_tokens[2].get('token')
    user3 = user_tokens[2].get('username')
    body = {"username": user3, "supermarket_id": supermarket_id, "shop_time": '2021-02-11 10:20'}
    response = test_client.post('/booking',  headers={'Authorization': 'JWT ' + auth_user3}, json=body)
    assert response.status_code == 201